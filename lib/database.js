...
module.exports = {
  ...
  get_last_txs: function(count, min, cb) {
    Tx.find({"$and": [ { 'total': {$gt: min} }, { vin: { $gt: [] } } ]}).sort({_id: 'desc'}).limit(count).exec(function(err, txs){
      if (err) {
        return cb(err);
      } else {
        return cb(txs);
  ...
  get_last_pos: function(count, cb) {
    Tx.find({ vin: [] }).sort({_id: 'desc'}).limit(count).exec(function(err, txs){
      if (err) {
        return cb(err);
      } else {
        return cb(txs);
